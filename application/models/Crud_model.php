...
class Crud_model extends CI_Model
{
    ...
    public function update_course($course_id, $type = "")
    {
        ...
        $data['batch1'] = $this->input->post('batch1');
        $data['batch2'] = $this->input->post('batch2');
        $data['batch3'] = $this->input->post('batch3');
        ...
        if($dataInfo[0]['file_name'] != ''){
			$data['course_companies_image_one'] = $dataInfo[0]['file_name'];
		}
		if($dataInfo[1]['file_name'] != ''){
			$data['course_companies_image_two'] = $dataInfo[1]['file_name'];
		}
		if($dataInfo[2]['file_name'] != ''){
			$data['course_companies_image_three'] = $dataInfo[2]['file_name'];
		}
		if($dataInfo[3]['file_name'] != ''){
			$data['course_companies_image_four'] = $dataInfo[3]['file_name'];
		}
        
        $data['why_learn_course'] = $this->input->post('why_learn_course');
        ...
        if($this->input->post('user_id') == ''){
			$course_uid = $this->get_course_by_id($course_id)->row_array();
			$data['user_id'] = $course_uid['user_id'];
		}else{
    ...
    public function add_lesson()
    {
        ...
        }elseif ($lesson_type == "textcontent"){
            $data['text_content'] = $this->input->post('editor1');
        } else {
    ...
    public function edit_lesson($lesson_id) {
        ...
        } else if($lesson_type == "textcontent"){
            $data['text_content'] = $this->input->post('editor1');
        }else {
            if ($attachment_type == 'iframe') {
                if (empty($this->input->post('iframe_source'))) {
        ...
        if (!empty($purchased_courses)) {
             foreach ($purchased_courses as $purchased_course) {
                ...
                if($partial_amount != 'undefined' && $partial_amount !=''){
                    $data['partial_amount_paid'] = $partial_amount;
                }else{
                    $data['partial_amount_paid'] = $amount_paid;
                }
                ...
                $data['date_added'] = strtotime(date('D, d-M-Y'));
                //$data['batch_date'] = $this->session->userdata('batch_date');
                $this->db->insert('payment', $data);
                
                $result = $this->db->query('select partial_amount_paid from payment where user_id='.$user_id.'')->row_array();
                if (!empty($result)) {
                    if ($result['partial_amount_paid'] == $data['amount']) {
                        $data = array('partial_payment' => '1');
                    }else{
                        $data = array('partial_payment' => '0');
                    }
                }else{
                    $partial_amount = $this->session->userdata('partial_amount');
                    if($partial_amount != 'undefined' && $partial_amount !=''){
            ...
            $partial_amount = $this->session->userdata('partial_amount');
            if (!empty($result)) {
                $partial_amount_old = $result['partial_amount_paid'];
                if($partial_amount != 'undefined' && $partial_amount !=''){
                    $data['partial_amount_paid'] = $partial_amount_old + $partial_amount;
                }
            }
            $update_payment = $this->db->where('user_id',$user_id)->update('payment', $data);
            $update_user = $this->db->where('id',$user_id)->update('users', array('partial_payment' => '1'));
        }
	...
	public function save_promocode()
    {
        ...
        if ($this->db->get_where('promocode', $data)->num_rows() > 0) {
            $this->session->set_flashdata('error_message', get_phrase('promocode_already_exist'));
        } else {
            $this->db->insert('promocode', $data);
            $this->session->set_flashdata('flash_message', get_phrase('promocode_added_sucessfully'));
        }
    }
	...
	function validate_promocode($promocode){
		return $this->db->get_where('promocode', array('promocode' => $promocode))->row_array();
    }
    function validate_emp_code($emp_code){
		//$otherdb = $this->load->database('otherdb', TRUE);
		//$sql = "SELECT * FROM en_users WHERE emp_code='".$emp_code."'";
        //$result = $otherdb->query($sql)->row_array();
		//return $result;
    }
}