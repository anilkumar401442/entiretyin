...
class Email_model extends CI_Model {
	...
	public function send_mail_registered_user($to = ""){
		$to_name = $this->db->get_where('users', array('email' => $to))->row_array();
		$email_data['subject'] = "SuccessFull Registration";
		$email_data['from'] = get_settings('system_email');
		$email_data['to'] = $to;
		//$email_data['to_name'] = $to_name['first_name'].' '.$to_name['last_name'];
		$email_data['receiver_name'] = $to_name['first_name'].' '.$to_name['last_name'];
		$email_data['message_body'] = 'Welcome To Entiretyin, Your Succesfully Registered';
		$email_template = $this->load->view('email/template', $email_data, TRUE);
		$header  = "MIME-Version: 1.0\r\n";
        $header .= "Content-type: text/html; charset=utf-8\r\n";
		$header .= "From:".$email_data['from'];
	    mail($email_data['to'],$email_data['subject'],$email_template,$header);
		//$this->send_smtp_mail($email_template, $email_data['subject'], $email_data['to'], $email_data['from'], 'verification');
	}
