...
<section class="course-content-area">
  <div class="container">
    <div class="row">
      <div class="col-lg-8">
          <?php if($course_details['batch1'] != '0000-00-00'){ ?>
        <div class="what-you-get-box" style="text-align: center;">
          <div class="what-you-get-title"><?php echo site_phrase('New_Batches_Start_From'); ?></div>
            <?php  $date = date('Y-m-d'); if (($course_details['batch1'] != '') && ($course_details['batch1'] > $date)) { ?>
              <button class="btn btn-primary" onClick="select_bacth(<?php echo $course_details['batch1']; ?>)"><?php echo date('d-m-Y',strtotime($course_details['batch1'])); ?></button>
            <?php } ?>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
            <?php  $date = date('Y-m-d'); if (($course_details['batch2'] != '') && ($course_details['batch2'] > $date)) { ?>
              <button class="btn btn-primary"><?php echo date('d-m-Y',strtotime($course_details['batch2'])); ?></button>
            <?php } ?>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
            <?php  $date = date('Y-m-d'); if (($course_details['batch3'] != '') && ($course_details['batch3'] > $date)) { ?>
              <button class="btn btn-primary"><?php echo date('d-m-Y',strtotime($course_details['batch3'])); ?></button>
            <?php } ?>
        </div>
        <?php } ?>
...
<?php if ($course_details['video_url'] != ""):
  ...
  <div class="modal fade" id="CoursePreviewModal" tabindex="-1" role="dialog" aria-hidden="true" data-keyboard="false" data-backdrop="static">
    ...
    function select_bacth(str) {
      $.ajax({
        type: "POST",
        url: "<?php echo base_url();?>home/select_bacth",
        data:'batch='+str,
