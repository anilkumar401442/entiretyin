...
class User extends CI_Controller {
    ...
    public function instructor_authorization($method) {
        ...
        if ($this->session->userdata('is_instructor') != 1) {
            $unprotected_routes = ['become_an_instructor', 'manage_profile', 'save_course_progress','zoom_meeting','getUserlivestatus','add_history'];
    ...
    //     $setting = $this->user_model->get_setting($userid);
    ...
    //             );
    //         $this->user_model->updateCredentials($this->input->post('uid'),$data_insert);
    //         $setting = $this->user_model->get_setting($userid);
    //         $page_data['settings']=$setting;
    //         $this->session->set_flashdata('flash_message', get_phrase('credentials_updated_succesfully'));
    //         $this->load->view('backend/index', $page_data);
    //     }
    //     else if($this->input->post('zoom_api_key')){
    //         $data_insert = array(
    //             'user_id'         => $userid,
    //             'zoom_api_key'    => $this->input->post('zoom_api_key'),
    //             'zoom_api_secret' => $this->input->post('zoom_api_secret'),
    //         );
    //         $this->user_model->addZoomSettingData($data_insert);
    //         $setting = $this->user_model->get_setting($userid);
    //         $page_data['settings']=$setting;
    //         $this->session->set_flashdata('flash_message', get_phrase('credentials_added_succesfully'));
    //         $this->load->view('backend/index', $page_data);
    //     }else{
    //         $page_data['settings']=$setting;
    //         $this->load->view('backend/index', $page_data);
    ...
    public function meetings(){
       ...
       // echo '<pre>';print_r($this->session->all_userdata()); exit;
            ...
            $page_data['page_name'] = 'meetings';
            $page_data['page_title'] = get_phrase('meetings');
            $page_data['courses']=$this->user_model->getCourses($userid);
            $page_data['users']=$this->user_model->getUserDetails($userid);
            $page_data['roleid']= $roleid;
            $page_data['meeting_data']=$this->user_model->getMeetingData($userid);
            //echo '<pre>';print_r($page_data['meeting_data']);exit;
            $this->load->view('backend/index', $page_data);
        }

    public function addLiveMeeting(){
        //ini_set('display_errors',1);
        $api_type = 'self';
        $userid=$this->session->userdata('user_id');
        $params = array(
        ...
        if ($response) {
            if (isset($response->id)) {
                ...
                $this->user_model->addData($insert_array);
                $this->session->set_flashdata('flash_message', get_phrase('meeting_added_successfully'));
                redirect('User/meetings');
            }else{
                $this->session->set_flashdata('error_message', get_phrase('please_check_and_add_meeting'));
                redirect('User/meetings');
            }
        }
        else{
            $this->session->set_flashdata('error_message', get_phrase('please_add_credentials'));
            redirect('User/meetings');
        }
    }
    ...
    public function getlivestatus(){
        //ini_set('display_errors',1);
        //echo '<pre>'; print_r($_POST);livestatus
        $data['page_name'] = 'livestatus';
        $data['page_title'] = get_phrase('livestatus');
        $userid=$this->session->userdata('user_id');
        $conference_id = $this->input->post('id');
        $data['conference_setting'] = $this->user_model->get_setting();
        $live          = $this->user_model->getMeetingJoinData($conference_id);
        $zoomsetting = $this->user_model->get_setting();
        //echo '<pre>';print_r($zoomsetting);exit;
        if (!empty($zoomsetting)) {
            $zoom_api_key    = $zoomsetting[0]['zoom_api_key'];
            $zoom_api_secret = $zoomsetting[0]['zoom_api_secret'];
        }
        $params = array(
        ...
        $this->load->library('zoom_api', $params);
        $meetingID               = json_decode($live[0]->return_response)->id;
        $api_Response            = $this->zoom_api->getMeeting($meetingID);
        //echo '<pre>'; print_r($api_Response);exit;
        $data['api_Response']    = $api_Response;
        $data['logged_staff_id'] = $this->session->userdata('user_id');
        $data['live']            = $live;
        $data['live_url']        = json_decode($live[0]->return_response);
        $data['page']            = $this->load->view('backend/user/livestatus', $data, true);
        $array                   = array('status' => '1', 'page' => $data['page']);
        echo json_encode($data);
    }

    public function zoom_meeting(){
        $page_data['page_name'] = 'zoom_meeting';
        $page_data['page_title'] = get_phrase('zoom_meeting');
        $userid=$this->session->userdata('user_id');
        $page_data['meetingList']=$this->user_model->getUserMeetingDetails($userid);
        //echo '<pre>'; print_r($page_data['meetingList']);exit();
        $this->load->view('backend/index', $page_data);
    }

    public function getUserlivestatus(){
        //ini_set('display_errors',1);
        $data['page_name'] = 'userlivestatus';
        $data['page_title'] = get_phrase('userlivestatus');
        $userid=$this->session->userdata('user_id');
        $conference_id = $this->input->post('id');
        $live=$this->user_model->getMeetingJoinData($conference_id);
        ...
        $data['conference_setting'] = $this->user_model->get_setting();
        $zoomsetting=$this->user_model->get_setting();
        //echo '<pre>'; print_r($zoomsetting[0]['zoom_api_key']);exit;
        if (!empty($zoomsetting)) {
                $zoom_api_key    = $zoomsetting[0]['zoom_api_key'];
                $zoom_api_secret = $zoomsetting[0]['zoom_api_secret'];
            }
            ...
            $data['live']            = $live;
            $data['live_url']        = json_decode($live[0]->return_response);
            $data['page']            = $this->load->view('backend/user/userlivestatus', $data, true);
            $array                   = array('status' => '1', 'page' => $data['page']);
            echo json_encode($data);
    }
    ...
    public function chgstatus(){
        ...
        $chgstatus=$this->input->post('chg_status');

        $insert_array = array(
            'status' => $this->input->post('chg_status'),
        );
            $insert_id = $this->user_model->updateData($conference_id, $insert_array);
            $response  = array('status' => 1, 'message' => 'update_message');
            redirect(site_url('user/meetings'), 'refresh');
    ...
    public function add_history(){
       // echo '<pre>'; print_r($_POST);exit;
        ...
        $live = $this->user_model->getMeetingJoinData($conference_id);
        // echo '<pre>'; print_r($live[0]->created_id);exit;
        if ($live[0]->created_id != $user_id) {
            $data_insert = array(
                'class_id' => $conference_id,
                'user_id'      => $user_id,
            );
            $this->user_model->updatehistory($data_insert); 
        }
            $array = array('status' => 1, 'success' => 'Inserted Successfully');
            echo json_encode($array);
    }

    public function meetings_report(){
        $page_data['page_name'] = 'meetings_report';
        $page_data['page_title'] = get_phrase('meetings_report');
        $userid=$this->session->userdata('user_id');
        $page_data['meetingList']=$this->user_model->getMeetingReportData($userid);
        //echo '<pre>'; print_r($page_data['meetingList']);exit;
        $this->load->view('backend/index', $page_data);
    }
    ...
    public function getViewerList(){
        ...
        //echo '<pre>'; print_r($recordid);
        $data['viewerDetail']=$this->user_model->getJoinedUsersData($recordid);
        $data['page'] = $this->load->view('backend/user/partialviewerlist', $data, true);
        $array                   = array('page' => $data['page']);
        echo json_encode($data);
        //echo '<pre>'; print_r($data['viewerDetail']);exit;
    }
