...
class Home extends CI_Controller {
    ...
    //     $api_key = '7eb503591dc742ab252384b5967868f8504a3c91bc5276307358c6a8c63d4f9d';
        
    //     $data = file_get_contents("http://api.ipinfodb.com/v3/ip-country/?key=$api_key&ip=$ipaddress&format=json");
    ...
    public function shopping_cart() {
        ...
        if ($this->session->userdata('partial_amount')) {
            $this->session->unset_userdata('partial_amount');
        }
    ...
    public function course_us($slug = "", $course_id = "") {
        $this->access_denied_courses($course_id);
        //echo $course_id;exit;
        $page_data['course_id'] = $course_id;
        $page_data['page_name'] = "course_page_us";
        $page_data['page_title'] = site_phrase('course');
        $this->load->view('frontend/'.get_frontend_settings('theme').'/index', $page_data);
    }
    ...
    public function handleCartItems($return_number = "") {
		...
		$reg_track_data = array(
		'user_id' => $this->session->userdata('user_id'),
		'course_id' => $course_id
		);
		$is_track_present = $this->user_model->is_track_present($course_id,$this->session->userdata('user_id'))->row_array();
		if(empty($is_track_present)){
			$this->user_model->add_registration_track($reg_track_data);
		}
    ...
    public function refund_and_cancel() {
        $page_data['page_name'] = 'refund_and_cancel';
        $page_data['page_title'] = site_phrase('refund_and_cancellation_policy');
        $this->load->view('frontend/'.get_frontend_settings('theme').'/index', $page_data);
    }
    ...
    public function submit_quiz($from = "") {
        ...
        foreach ($quiz_questions as $quiz_question) {
            ...
            $result = $this->db->query('select * from question_answers where user_id='.$this->session->userdata('user_id').' AND question_id='.$quiz_question['id'].'')->row_array();
            if($result){
                $res = $this->db->where(array('user_id'=>$this->session->userdata('user_id'),'question_id'=>$quiz_question['id']))->update('question_answers',$container);
            }else{
                $res = $this->db->insert('question_answers',$container);
            }
        }
        $result = $this->db->query('select count(id) as total_questions,sum(submitted_answer_status)  as total_score from question_answers where user_id='.$this->session->userdata('user_id').'')->row_array();
        $percentage = ($result['total_score'] / $result['total_questions']) * 100;

        $lesso = $this->db->query('select course_id from lesson where id='.$quiz_id.'')->row_array();

        $page_data['cour_id']   = $lesso['course_id'];
        $page_data['quiz_percentage']   = $percentage;
    ...
    public function payment(){
		...
		if(isset($promo_amount)){
			$total_payable = $this->session->userdata('total_price_of_checking_out')-$promo_amount;
		}else{
			$total_payable = $this->session->userdata('total_price_of_checking_out');
		}
		
		$partial_amount = $this->session->userdata('partial_amount');
        if($partial_amount != 'undefined' && $partial_amount !=''){
            $total_payable = $partial_amount;
        }
		
        $key = 'rzp_live_FfhecYxryawVkz';
		$secret = 'drrHoMpjC9ubi0ujTAaM5lrG';
        ...
        if($this->session->userdata('country') == 'United States of America'){
			$currency = "USD"; 
		}else{
		    $currency = "INR";
		}
		...
		$razorpayOrder = $api->order->create(array(
			'receipt'         => rand(),
			'amount'          => $total_payable * 100, // 2000 rupees in paise
			'currency'        => $currency,
	...
	{
		$user_data = $this->db->get_where('users', array('id' => $this->session->userdata('user_id')))->row_array();

		$data = array(
			"key" => 'rzp_live_FfhecYxryawVkz',
			"amount" => $amount,
			"name" => "Entirety Pvt Ltd",
			"description" => "Learn To Code",
			"image" => "https://entiretyin.com/assets/images/ent/hnlogo.png",
			"prefill" => array(
	...
	{   
		...
		if ($success === true) {
            ...
            $this->crud_model->course_purchase($this->session->userdata('user_id'), 'razorpay', $this->session->userdata('total_price_of_checking_out'), $_POST['razorpay_payment_id'], $_POST['razorpay_signature']);
			$this->session->unset_userdata('cart_items');
			$this->session->unset_userdata('promo_amount');
			$this->session->set_userdata('payment','success');
			redirect('home/my_courses', $page_data);
		}
		else {
			$this->session->set_flashdata('error_message', site_phrase('session_time_out'));
			$this->session->set_userdata('payment','failed');
            redirect('home/shopping_cart', 'refresh');
		}
	}
	public function validate_promocode(){
		$return_array = array('status' => false, 'message' => '');
		$promocode = $this->input->post('promocode');
		$result = $this->crud_model->validate_promocode($promocode);
		if(!empty($result)){
			$this->session->set_userdata('promo_amount', $result['promo_amount']);
			$total_payable = $this->session->userdata('total_price_of_checking_out')-$result['promo_amount'];
			$return_array['status'] = true;
            $return_array['return_data'] = '<div class="total-price">'.currency($total_payable).'</div>';
		}else{
			$return_array['status'] = false;
		}
		echo json_encode($return_array);
	}
	public function validate_emp_code(){
		$return_array = array('status' => false, 'message' => '');
		$emp_code = $this->input->post('emp_code');
		if($emp_code != ''){
		    $result = $this->crud_model->validate_emp_code($emp_code);	
		}
		if(!empty($result)){
	...
	public function download_pdf($course_id){
        $user_id = $this->session->userdata('user_id');
        $result = $this->db->query('select count(id) as total_questions,sum(submitted_answer_status)  as total_score from question_answers where user_id='.$user_id.'')->row_array();
        $page_data['courses'] = $this->db->query('select title from course where id='.$course_id.'')->row_array();
        $percentage = ($result['total_score'] / $result['total_questions']) * 100;
        $page_data['user'] = $this->db->query('select * from users where id='.$user_id.'')->row_array();


        // $path = 'assets/images/vowel/vowel-logo.png';
        // $type = pathinfo($path, PATHINFO_EXTENSION);
        // $data = file_get_contents($path);
        // $this->data['image_base64'] = 'data:image/' . $type . ';base64,' . base64_encode($data);

        $this->load->view('lessons/pdf_quiz_result', $page_data);
    ...
    public function save_partial_amount_session(){
        $amount = $this->input->post('amount');
        $this->session->set_userdata('partial_amount',$amount);
    }
    public function save_partial_amount_homepage_modal(){
        $result = $this->db->query('select * from payment where user_id='.$this->session->userdata('user_id').'')->row_array();
        $amount = $result['amount']-$result['partial_amount_paid'];
        $this->session->set_userdata('partial_amount',$amount);
    }
    public function remove_home_popup(){
        $this->session->set_userdata('payment_status','1');
    }
    public function select_bacth(){
        $batch_date = $this->input->post('batch');
        $this->session->set_userdata('batch_date',$batch_date);
    }
}
