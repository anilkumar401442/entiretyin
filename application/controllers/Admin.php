...
class Admin extends CI_Controller {
    ...
    public function get_courses() {
        ...
        if(!empty($courses)) {
            foreach ($courses as $key => $row) {
                ...
                $duplicate_course = site_url('admin/duplicate_course/'.$row->id);
                ...
                <li><a class="dropdown-item" href="'.$duplicate_course.'">'.get_phrase("duplicate_this_course").'</a></li>
                </ul>
                </div>
                ';
	...
	public function registration_tracking($param1 = "", $param2 = "") {
        if ($this->session->userdata('admin_login') != true) {
            redirect(site_url('login'), 'refresh');
        }
		
        $page_data['page_name'] = 'registration_tracking';
		$page_data['page_title'] = get_phrase('Registration Tracking List');
		$page_data['courses'] = $this->user_model->get_regtrack_courses();
        $page_data['tracks'] = $this->user_model->get_regtracks();
        $this->load->view('backend/index', $page_data);
    }
	...
	public function promocode() {
		...
		$page_data['page_title'] = get_phrase('Promo Code');
		$page_data['promocode'] = $this->user_model->get_promcode();
        $this->load->view('backend/index', $page_data);
    }

    public function promocode_datatable() {
        if ($this->session->userdata('admin_login') != true) {
            redirect(site_url('login'), 'refresh');
        }
        ...
        $total_count = $this->user_model->get_promocode_count();
        $totalData = $total_count;
        $totalFiltered = $totalData; 
        
        if(empty($this->input->post('search')['value'])){            
					$posts = $this->user_model->get_promocode($limit,$start);
					//$totalFiltered = count($posts);
		}else {
					$search = $this->input->post('search')['value'];
					$posts = $this->Report_Model->get_promocode_list($limit,$start,$search);
					$totalFiltered = $this->Report_Model->get_promocode_list_count($search);
		}
        ...
        {   
            foreach ($posts as $post){
				$onclick = "confirm_modal('".site_url('admin/delete_promocode/'.$post['id'])."')";
                $action = '<a class="btn btn-primary" href="#" onclick='.$onclick.' >Delete</a>';				
                $nestedData['#'] = ++$start;
                $nestedData['promocode'] = $post['promocode'];
				$nestedData['promo_amount'] = $post['promo_amount'];
                $nestedData['date'] = date('d-m-Y',strtotime($post['created_on']));
	...
	public function registration_tracking_datatable() {
        if ($this->session->userdata('admin_login') != true) {
            redirect(site_url('login'), 'refresh');
        }
        ...
        $total_count = $this->user_model->get_registration_tracks_count();
        $totalData = $total_count;
        $totalFiltered = $totalData; 
		...
		//echo $date;exit;
        if($course_id !='' || $date != ''){
			$posts = $this->user_model->get_registration_tracks_filter($limit,$start,$course_id,$date);
			$totalFiltered = $this->user_model->get_registration_tracks_filter_count($course_id,$date);
		}else{
		    if(empty($this->input->post('search')['value'])){            
					$posts = $this->user_model->get_registration_tracks($limit,$start);
					//$totalFiltered = count($posts);
			}else {
					$search = $this->input->post('search')['value'];
					$posts = $this->Report_Model->get_registration_tracks_searched_list($limit,$start,$search);
					$totalFiltered = $this->Report_Model->get_registration_tracks_searched_list_count($search);
			}	
		}
        ...
        {   
            foreach ($posts as $post){ 
                ...
                $nestedData['#'] = ++$start;
                $nestedData['user'] = $post['first_name'];
				$nestedData['email'] = $post['email'];
				$nestedData['mobile'] = $post['mobile'];
                $nestedData['course'] = $post['title'];
                $nestedData['date'] = date('d-m-Y',strtotime($post['createdon']));              
                $data[] = $nestedData;
	...
	public function trackExport() {
        ...
        $listInfo = $this->user_model->trackExport($course_id,$date);
        ...
        // set Row
        $rowCount = 2;
        foreach ($listInfo as $post) {
            
            $objPHPExcel->getActiveSheet()->SetCellValue('A' . $rowCount, $post['first_name']);
            $objPHPExcel->getActiveSheet()->SetCellValue('B' . $rowCount, $post['email']);
            $objPHPExcel->getActiveSheet()->SetCellValue('C' . $rowCount, $post['mobile']);
            $objPHPExcel->getActiveSheet()->SetCellValue('D' . $rowCount, $post['title']);
            $objPHPExcel->getActiveSheet()->SetCellValue('E' . $rowCount, date('d-m-Y',strtotime($post['createdon'])));
            $rowCount++;
        }

        $filename = "Registration Track Report ". date("Y-m-d").".xlsx";
	...
	public function add_promocode() {
        ...
        $page_data['page_title'] = get_phrase('add_promocode');
        $this->load->view('backend/index', $page_data);
    }
	public function save_promocode(){
		$this->crud_model->save_promocode();
		redirect(site_url('admin/promocode'), 'refresh');
	}
	public function delete_promocode($id){
		$this->crud_model->delete_promocode($id);
		redirect(site_url('admin/promocode'), 'refresh');
	}
	public function duplicate_course($course_id=""){
		$course_detail = $this->crud_model->get_course_by_id($course_id)->row_array();
		$data = array(
		'title' => $course_detail['title'],
		'short_description' => $course_detail['short_description'],
		'description' => $course_detail['description'],
		'outcomes' => $course_detail['outcomes'],
		'language' => $course_detail['language'],
		'category_id' => $course_detail['category_id'],
		'sub_category_id' => $course_detail['sub_category_id'],
		'section' => $course_detail['section'],
		'requirements' => $course_detail['requirements'],
		'price' => $course_detail['price'],
		'discount_flag' => $course_detail['discount_flag'],
		'discounted_price' => $course_detail['discounted_price'],
		'level' => $course_detail['level'],
		'user_id' => $course_detail['user_id'],
		'thumbnail' => $course_detail['thumbnail'],
		'video_url' => $course_detail['video_url'],
		'date_added' => $course_detail['date_added'],
		'last_modified' => $course_detail['last_modified'],
		'course_type' => $course_detail['course_type'],
		'is_top_course' => $course_detail['is_top_course'],
		'is_admin' => $course_detail['is_admin'],
		'status' => $course_detail['status'],
		'course_overview_provider' => $course_detail['course_overview_provider'],
		'meta_keywords' => $course_detail['meta_keywords'],
		'meta_description' => $course_detail['meta_description'],
		'is_free_course' => $course_detail['is_free_course']
		);
		$this->db->insert('course', $data);
		$this->session->set_flashdata('flash_message', get_phrase('course_duplicated_successfully'));
		redirect(site_url('admin/courses'), 'refresh');
	}
	public function delete_course_company_image($course_id = "", $param1 = ""){
		$id = $course_id;
		$typ = $param1;
		$col = 'course_companies_image_'.$typ.'';
		$data = array(
		$col => ''
		);
		$res = $this->db->where('id', '1')->update('course', $data);
		redirect('admin/course_form/course_edit/'.$course_id);
	}
	public function pending_payments($param1 = "", $param2 = "") {
        if ($this->session->userdata('admin_login') != true) {
            redirect(site_url('login'), 'refresh');
        }
        
        $page_data['page_name'] = 'pending_payments';
        $page_data['page_title'] = get_phrase('Pending Payments List');
        $page_data['users'] = $this->db->query('select users.*,payment.*,course.title from users left join payment on users.id=payment.user_id left join course on course.id=payment.course_id where users.partial_payment=0')->result_array();
        $this->load->view('backend/index', $page_data);
    }
}