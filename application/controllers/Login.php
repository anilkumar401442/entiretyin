...
class Login extends CI_Controller {
    ...
    public function validate_login($from = "") {
        ...
        if ($query->num_rows() > 0) {
            ...
            $this->session->set_userdata('payment_status', $row->partial_payment);
    ...
    public function register() {
        ...
        if(empty($data['first_name']) || empty($data['email']) || empty($data['password']) || empty($data['country'])){
        ...
        if($validity === 'unverified_user' || $validity == true) {
            if($validity === true){
                ...
                $this->email_model->send_mail_registered_user($data['email']);
            }else{
